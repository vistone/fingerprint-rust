...
pub fn extract_signature(spec: &ClientHelloSpec) -> ClientHelloSignature {
    ...
    if let Some(ref metadata) = spec.metadata {
        ...
        if let Some(ext_meta) = metadata
            .extension_metadata
            .get(&crate::dicttls::extensions::EXT_TYPE_SUPPORTED_GROUPS)
        {
        ...
        if let Some(ext_meta) = metadata
            .extension_metadata
            .get(&crate::dicttls::extensions::EXT_TYPE_EC_POINT_FORMATS)
        {
        ...
        if let Some(ext_meta) = metadata
            .extension_metadata
            .get(&crate::dicttls::extensions::EXT_TYPE_SIGNATURE_ALGORITHMS)
        {
