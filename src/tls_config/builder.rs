...
use crate::dicttls::{
    ...
    supported_groups::{GREASE_PLACEHOLDER as GREASE_SG, CURVE_P256, CURVE_P384, X25519, X25519_MLKEM768},
};
...
use crate::tls_config::spec::{ClientHelloSpec, POINT_FORMAT_UNCOMPRESSED, PSK_MODE_DHE, RENEGOTIATE_ONCE_AS_CLIENT, CERT_COMPRESSION_BROTLI, VERSION_TLS12, VERSION_TLS13};
    ...
    pub fn chrome_133_extensions() -> (Vec<Box<dyn TLSExtension>>, crate::tls_config::metadata::SpecMetadata) {
        ...
        let extensions: Vec<Box<dyn TLSExtension>> = vec![
            ...
            Box::new(ALPNExtension::new(
                Self::chrome_alpn_protocols().iter().map(|s| s.to_string()).collect()
            )),
