...
use crate::dns::types::{DNSResult, DomainIPs, DNSError, IPInfo};
...
impl DNSResolver {
    ...
    pub async fn resolve(&self, domain: &str) -> Result<DNSResult, DNSError> {
        eprintln!("[DNS Resolver] ========== 开始解析域名: {} ==========", domain);
        ...
        if let Ok(ipv4_addrs) = self.resolve_aaaa_or_a(domain, false).await {
            eprintln!("[DNS Resolver] IPv4 解析成功，获得 {} 个地址", ipv4_addrs.len());
        ...
        if let Ok(ipv6_addrs) = self.resolve_aaaa_or_a(domain, true).await {
            eprintln!("[DNS Resolver] IPv6 解析成功，获得 {} 个地址", ipv6_addrs.len());
            domain_ips.ipv6 = ipv6_addrs;
        } else {
            eprintln!("[DNS Resolver] IPv6 解析失败");
        }

        eprintln!("[DNS Resolver] ========== 域名解析完成: {} (IPv4: {} 个, IPv6: {} 个) ==========", 
                 domain, domain_ips.ipv4.len(), domain_ips.ipv6.len());
    ...
    async fn resolve_with_hickory(
        ...
        let servers_with_sockets: Vec<_> = servers.iter()
            .filter_map(|server_str| {
                ...
                let (ip_str, port) = if let Some(colon_pos) = server_str.find(':') {
                    ...
                    let port = server_str[colon_pos + 1..].parse::<u16>()
                        .unwrap_or(53);
                    (ip.to_string(), port)
                } else {
        ...
        eprintln!("[DNS Resolver] 单个服务器超时: {:?}, 总体超时: {:?}", opts.timeout, self.timeout);
        ...
        eprintln!("[DNS Resolver] 开始收集查询结果（总体超时: {:?}，总服务器数: {}）", overall_timeout, total_servers);
        ...
        let ip_infos: Vec<IPInfo> = all_ips.into_iter()
            .map(|ip| IPInfo::new(ip))
            .collect();
        
        eprintln!("[DNS Resolver] 转换为 IPInfo，最终返回 {} 个唯一 IP 地址（已去重）", ip_infos.len());
