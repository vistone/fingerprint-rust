...
impl TestResult {
    fn success(profile_name: String, protocol: String, status_code: u16, response_size: usize, duration_ms: u64) -> Self {
        Self {
            profile_name,
            protocol,
            success: true,
...
fn test_single_profile_http1(
    ...
    if !response.http_version.contains("HTTP/1.1") {
        return Err(format!("协议不匹配: 期望 HTTP/1.1，实际 {}", response.http_version));
    }
...
fn test_single_profile_http2(
    ...
    if !response.http_version.contains("HTTP/2") {
        return Err(format!("协议不匹配: 期望 HTTP/2，实际 {}", response.http_version));
    }
...
fn test_single_profile_http3(
    ...
    if !response.http_version.contains("HTTP/3") {
        return Err(format!("协议不匹配: 期望 HTTP/3，实际 {}", response.http_version));
    }
...
fn print_summary(protocol: &str, results: &[TestResult], total_duration: Duration) {
    ...
    if !success.is_empty() {
        let avg_time: f64 = success.iter().map(|r| r.duration_ms as f64).sum::<f64>() / success.len() as f64;
    ...
    if !failed.is_empty() {
        ...
        for result in failed.iter().take(10) {
            println!("    - {}: {}", result.profile_name, result.error.as_ref().unwrap_or(&"未知错误".to_string()));
        }
        if failed.len() > 10 {
