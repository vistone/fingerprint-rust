...
fn test_tls_handshake_builder() {
    ...
    #[cfg(feature = "crypto")]
    {
        ...
        let client_hello_result = TLSHandshakeBuilder::build_client_hello(
            &spec,
            "example.com"
        );
        ...
        #[cfg(feature = "export")]
        {
            println!("✅ ClientHello (hex): {}", hex::encode(&client_hello[..std::cmp::min(64, client_hello.len())]));
        }
        #[cfg(not(feature = "export"))]
        {
            println!("✅ ClientHello (前64字节): {:?}", &client_hello[..std::cmp::min(64, client_hello.len())]);
        }
    }
...
fn test_browser_profiles() {
    ...
    for (name, profile) in profiles {
        ...
        assert!(!pseudo_order.is_empty(), "{} 应该有 Pseudo Header Order", name);
        
        println!("✅ {}: ClientHelloID={}, Settings={}, PseudoHeaders={}", 
            name, 
            client_id,
            settings.len(),
            pseudo_order.len()
...
fn test_user_agent_generation() {
    ...
    if ua2.is_ok() {
        ...
        assert!(ua2.contains("Firefox") || ua2.contains("Gecko"), "应该包含 Firefox 或 Gecko");
    } else {
...
fn test_http_headers_generation() {
    ...
    assert!(!headers.accept_language.is_empty(), "应该有 Accept-Language");
...
fn test_cookie_parsing() {
    ...
    let set_cookie = "session_id=abc123; Path=/; Domain=example.com; Secure; HttpOnly; SameSite=Strict";
...
fn test_http2_settings() {
    ...
    assert!(!firefox_settings.is_empty(), "Firefox 应该有 HTTP/2 Settings");
...
fn test_full_integration() {
    ...
    // 4. 构建请求
    let _request = HttpRequest::new(HttpMethod::Get, "https://example.com/")
