...
impl ValidationStats {
    ...
    fn print_summary(&self) {
        ...
        if self.http1_1_success > 0 {
            println!(
                "  - 平均响应时间: {}ms",
                self.total_time_ms / self.http1_1_success as u64
            );
        }
        println!();
    }
...
/// 验证单个指纹
fn validate_single_fingerprint(profile_name: &str, profile: &ClientProfile) -> ValidationResult {
    // 1. 验证指纹配置
    ...
    println!(
        "  📌 指纹配置: {}",
        if fingerprint_valid { "✅" } else { "❌" }
    );
    println!(
        "  📌 TLS 配置: {}",
        if tls_config_valid { "✅" } else { "❌" }
    );

    // 3. 创建 HTTP 客户端
    let headers = HTTPHeaders::default();
    let client = HttpClient::with_profile(profile.clone(), headers, user_agent);
...
fn validate_fingerprint_config(profile: &ClientProfile) -> bool {
    match profile.get_client_hello_spec() {
        Ok(spec) => {
            // 打印调试信息
            eprintln!(
                "  DEBUG: cipher_suites={}, extensions={}, tls_vers_min={}, tls_vers_max={}",
    ...
    match profile.get_client_hello_spec() {
        Ok(spec) => {
            ...
            if spec.cipher_suites.len() < 5 {
                eprintln!(
                    "  FAIL TLS: cipher_suites too few: {}",
                    spec.cipher_suites.len()
                );
                return false;
            }
        ...
        match client.get(url) {
            Ok(response) => {
                ...
                if response.is_success() {
                    return TestResult::success(response.status_code, elapsed, response.body.len());
                }
            }
            Err(_) => {
                // 尝试下一个 URL
        ...
        for result in results {
            ...
            if result.http1_1_result.success {
                writeln!(
                    file,
                    "    状态码: {}",
                    result.http1_1_result.status_code.unwrap()
                )
                .unwrap();
                writeln!(
                    file,
                    "    响应时间: {}ms",
                    result.http1_1_result.response_time_ms
                )
                .unwrap();
                writeln!(
                    file,
                    "    响应大小: {} 字节",
                    result.http1_1_result.response_size
                )
                .unwrap();
            } else {
                writeln!(
