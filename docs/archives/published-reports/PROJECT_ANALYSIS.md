...
- 💡 **优化方案**: 可以移除 libpcap 依赖，使用纯 Rust 实现
  - **当前状况**: 仅用于实时网卡捕获（`Capture::from_device`）和 pcap 文件读取（`Capture::from_file`）
  - **已有基础**: 项目已实现完整的数据包解析器（`PacketParser`），可解析 IPv4/IPv6、TCP/UDP/ICMP
  - **替代方案**:
    1. **实时捕获**: 使用 `socket2` + raw socket（已在项目中使用）或 `pnet` crate
    2. **pcap 文件**: 使用纯 Rust 的 `pcap-file` 或 `pcap-parser` crate
  - **优势**: 移除系统依赖，跨平台编译更简单，部署更容易
  - **工作量**: 中等（2-3天），主要修改 `capture/mod.rs`
...
#### 移除 libpcap 系统依赖
- 📝 **目标**: 使用纯 Rust 实现替代 libpcap，移除系统依赖
- 📝 **实现方案**:
  1. **pcap 文件读取**: 使用 `pcap-file` 或 `pcap-parser` crate（纯 Rust）
  2. **实时捕获**: 使用 `pnet` crate 或直接使用 `socket2` raw socket
  3. **修改范围**: 仅需修改 `crates/fingerprint-defense/src/capture/mod.rs`（约 60 行）
- 📝 **优势**:
  - 移除系统依赖，简化编译和部署
  - 提升跨平台兼容性（Windows 无需 WinPcap）
  - 更好的 CI/CD 体验（无需安装系统包）
- 📝 **工作量**: 2-3 天

**理由**: 项目已有完整的数据包解析器（`PacketParser`），只需替换捕获层即可
...
1. **移除系统依赖**
   - 使用纯 Rust 的 pcap-file/pcap-parser 替代 libpcap
   - 使用 pnet 或 raw socket 实现实时捕获
   - 简化跨平台编译和部署
