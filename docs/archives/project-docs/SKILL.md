...
### Workspace Structure: 20 Specialized Crates

```
fingerprint-rust/
├── Core Foundation (2 crates)
│   ├── fingerprint-core          # System-level protection core abstractions
│   ├── fingerprint-tls           # TLS configuration, extensions, handshakes
│
├── Profile & Configuration (2 crates)
│   ├── fingerprint-profiles      # Browser fingerprint profiles module
│   ├── fingerprint-headers       # HTTP Headers and User-Agent generation
│
├── Protocol Support (2 crates)
│   ├── fingerprint-http          # HTTP client implementation with QUIC support
│   ├── fingerprint-dns           # DNS pre-parse service
│
├── Defense & Analysis (2 crates)
│   ├── fingerprint-defense       # System-level protection implementation
│   ├── fingerprint-anomaly       # Anomaly detection module
│
├── Browser API Fingerprinting (6 crates)
│   ├── fingerprint-canvas        # Canvas fingerprinting and obfuscation
│   ├── fingerprint-webgl         # WebGL fingerprinting module
...
│   ├── fingerprint-storage       # Storage fingerprinting module
│   ├── fingerprint-ml            # Machine learning fingerprint matching
│
├── Integration (2 crates)
│   ├── fingerprint-api-noise     # Browser API noise injection
...
#### **fingerprint-core**
System-level protection core abstract layer. From single service protection improve to system-level protection, provides system-level core abstractions and interface.

Core Features:
- System-level abstractions: system context, network traffic, protection decision
- Offense and defense unified interface: fingerprint abstractions, analysis interface, protection interface
- Core types and utilities: type definitions, metadata, utility functions
...
- Extension field handling (SNI, ALPN, etc.)
...
#### **fingerprint-http**
HTTP client implementation module supporting HTTP/1.1, HTTP/2, and HTTP/3 protocols. Also includes QUIC (RFC 9000) initial packet fingerprinting.
...
#### **fingerprint-defense**
System-level protection implementation layer, based on `fingerprint-core` system-level abstractions.

Implemented Features:
- ✅ Passive fingerprint identification (TCP/IP, HTTP, TLS)
- ✅ Cross-layer consistency validation (JA4+)
- ✅ Fingerprint database (SQLite storage)
- ✅ Learning mechanism (automatic discovery)
- ✅ Packet capture (pure Rust implementation)

Planned Features:
- System analyzer implementation
- System protector implementation
...
#### **fingerprint-ml**
机器学习指纹匹配模块 提供高级的指纹相似度计算和分类能力.

Core Functions:
- Fingerprint similarity calculation
- Classification algorithms
- Risk score computation
- Reference fingerprint management
