...
pub trait TLSExtension: std::fmt::Debug + Any {
    ...
    /// Checkextensionwhether为empty
    /// defaultimplement：length为 0  when 为empty
    fn is_empty(&self) -> bool {
        self.len() == 0
    }

    /// readextensioncount据 to bytesbuffer
    ...
    /// convert to Any trait object， for 向down转型
...
pub trait TLSExtensionWriter: TLSExtension {
    ///  from bytesbufferwriteextensioncount据
...
/// Checkwhether是 GREASE value
fn is_grease_uint16(v: u16) -> bool {
    // GREASE value的pattern：0x1a1a, 0x2a2a, 0x3a3a, ..., 0xfafa
    let low = v & 0xff;
    let high = (v >> 8) & 0xff;
