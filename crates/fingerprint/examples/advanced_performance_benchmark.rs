...
{
    println!("\n⏱️  Running benchmark: {} ({} iterations)...", name, iterations);
...
fn bench_comparative_analysis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("├────────────────────────────────────────────────────────────┤");
    println!("│ Approach 1 (New): {:.3} ms mean                       │", 
        stats1.mean.as_secs_f64() * 1000.0);
    println!("│ Approach 2 (Reuse): {:.3} ms mean                     │", 
...
fn bench_memory_profile() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("├────────────────────────────────────────────────────────────┤");
    println!("│ Profile struct:     {:>6} bytes                         │", estimated_profile_size);
    println!("│ ClientHello spec:   {:>6} bytes                         │", estimated_spec_size);
    println!("└────────────────────────────────────────────────────────────┘");
...
fn bench_regression_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("├────────────────────────────────────────────────────────────┤");
    println!("│ Baseline:    {:.3} μs                                  │", 
        baseline_mean.as_secs_f64() * 1_000_000.0);
    println!("│ Current:     {:.3} μs                                  │", 
        stats.mean.as_secs_f64() * 1_000_000.0);
    println!("│ Ratio:       {:.2}x                                        │", regression_ratio);
    println!("│ Tolerance:   {:.2}x                                        │", tolerance);
