            ...
            Ok((mut stream, addr)) => {
                ...
                #[cfg(target_os = "linux")]
                {
                    ...
                    unsafe {
                        ...
                        if getsockopt(fd, SOL_SOCKET, SO_RCVBUF, &mut rcvbuf as *mut _ as *mut libc::c_void, &mut len) == 0 {
...
fn test_tcp_client_with_profile(port: u16, tcp_profile: &fingerprint_core::tcp::TcpProfile) -> std::io::Result<()> {
...
fn test_tcp_profile_application() {
    ...
    // 测试不同的 TCP Profile
    let test_cases = vec![
        ("Windows", fingerprint_core::tcp::TcpProfile::for_os(fingerprint_core::types::OperatingSystem::Windows10)),
        ("macOS", fingerprint_core::tcp::TcpProfile::for_os(fingerprint_core::types::OperatingSystem::MacOS14)),
        ("Linux", fingerprint_core::tcp::TcpProfile::for_os(fingerprint_core::types::OperatingSystem::Linux)),
    ];
