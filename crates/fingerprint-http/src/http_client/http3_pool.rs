    ...
    let send_request_mutex = session_pool
        .get_or_create_session(&key, async {
            ...
            let quic_client_config = quinn::crypto::rustls::QuicClientConfig::try_from(tls_config)
                .map_err(|e| HttpClientError::Http3Error(format!("QUIC TLS config error: {}", e)))?;
            ...
            transport_config.max_idle_timeout(Some(
                quinn::IdleTimeout::try_from(std::time::Duration::from_secs(60)).map_err(|e| {
                    HttpClientError::Http3Error(format!("configurationtimeoutfailure: {}", e))
                })?,
            ));
            ...
            let mut endpoint = quinn::Endpoint::client(bind_addr).map_err(|e| {
                HttpClientError::Http3Error(format!("Create endpoint failure: {}", e))
            })?;
            ...
            let connection = connecting.await.map_err(|e| {
                HttpClientError::Http3Error(format!("establishConnection failed: {}", e))
            })?;

            // establish HTTP/3 connection
            let quinn_conn = h3_quinn::Connection::new(connection);

            let (driver, send_request) = h3::client::new(quinn_conn).await.map_err(|e| {
                HttpClientError::Http3Error(format!("HTTP/3 handshakefailure: {}", e))
            })?;
    ...
    if let Some(body) = &request.body {
        if !body.is_empty() {
            stream
                ...
                .await
                .map_err(|e| {
                    HttpClientError::Http3Error(format!("Failed to send request body: {}", e))
                })?;
        }
    }
