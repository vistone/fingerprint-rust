//! HTTP/3 sessionpool
//!
//! pool化 h3::client::SendRequest 句柄，implement真正 HTTP/3 多路复用
//! 避免每次request都重新进行 QUIC handshake and HTTP/3 connection建立
...
struct H3Session {
    ...
    /// back台任务句柄（ for 管理 h3 connection驱动）
        ...
        >,
    {
        // try from pool中Get
        {
            let mut sessions = self.sessions.lock().unwrap_or_else(|e| {
                eprintln!("warning: H3 sessionpool锁failure: {}", e);
                drop(e.into_inner());
                self.sessions.lock().expect("unable toGet H3 sessionpool锁")
            });
        ...
        // Checkwhether正 in Create中 (Race Condition Fix)
        let rx = {
        ...
        // 无论successfailure，都 from  pending 中remove
