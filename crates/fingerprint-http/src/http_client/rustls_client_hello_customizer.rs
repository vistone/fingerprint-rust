...
//!
//! explain：
//! - rustls 并不一定willsend spec 里列出的allextension；herewill以 rustls actual `used` 为准，
//!   onlypair交set做重排，并把not覆盖's extensions按 rustls defaultorder追加，ensure仍是anvalid的排列。
//! - spec 里mayappearmultiple GREASE extension（ in realbrowser中它们usually是different GREASE value）。
//!   为avoid"重复extensiontype"cause rustls refuse，wewill把each GREASE 占bit符map成different GREASE value。
//!
...
// whenusesupport ClientHelloCustomizer  rustls fork  when ，canceldown面的comment并enabled相close代码
...
/// - GREASE：把重复 GREASE 占bit符map成different GREASE value
fn desired_extension_ids_from_spec(spec: &ClientHelloSpec) -> Vec<u16> {
    ...
    for ext in &spec.extensions {
        ...
        // process GREASE：尽量给each GREASE allocatedifferent的value，以符合"多 GREASE extension"的现实形态。
