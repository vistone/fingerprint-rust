        ...
        for part in &parts[1..] {
            ...
            if part.to_lowercase().starts_with("domain=") {
                ...
                if let Ok(secs) = part[8..].parse::<u64>() {
                    ...
                    // 让 Max-Age 真正生效：convert to绝pair expires 以复用 is_expired()
...
impl CookieStore {
    ...
    ///
    /// # Parameters
    /// - `domain`: request的domain
    /// - `path`: request的path
    /// - `is_secure`: whether为 HTTPS connection（ for  Secure Cookie Check）
    pub fn generate_cookie_header(
        ...
        let matching_cookies: Vec<String> = cookies
            ...
            .filter(|c| {
                ...
                // securityFix: Secure Cookie 只能 in HTTPS connectionupsend
