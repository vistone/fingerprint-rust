 ...
 min_active_servers
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self, crate::dns::types::DNSError> {
        ...
        let content = fs::read_to_string(path).map_err(|e| {
            crate::dns::types::DNSError::Config(format!("unable toreadfile: {}", e))
        })?;
    ...
    pub fn save_to_file<P: AsRef<Path>>(&self, path: P) -> Result<(), crate::dns::types::DNSError> {
        ...
        fs::write(&temp_path, json_content).map_err(|e| {
            crate::dns::types::DNSError::Config(format!("unable towritefile: {}", e))
        })?;
    ...
    pub async fn health_check_and_save_incremental(
                            ...
                            if ip_count > 0 {
                                ...
                                if current_count.is_multiple_of(save_batch_size) {
                                    ...
                                    if let Err(e) = pool.save_default() {
                                        ...
                                        eprintln!(
                                            "alreadysave {} availableserver to file",
                                            current_count
                                        );
                                    }
                                }
        ...
        if !final_servers.is_empty() {
            ...
            if let Err(e) = pool.save_default() {
                ...
                eprintln!(
                    "最finalsave了 {} availableserver to file",
                    final_servers.len()
                );
            }
        }
