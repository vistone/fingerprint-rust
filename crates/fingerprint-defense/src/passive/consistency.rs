...
//!
//! crossValidate TCP、TLS and HTTP layercountdata，detect欺骗behavior and abnormal machine er person 。
...
impl ConsistencyAnalyzer {
 /// analysistrafficin many layerconsistency
 ...
 if ua.contains(" window s") &&!tcp_os.contains(" window s") && tcp_os.contains("linux") {
 report.add_discrepancy(
 "TCP stack identify as Linux，but HTTP User-Agent 声称 is Windows".to_string(),
 50,
);
 }
 ...
 {
 report.add_discrepancy(
 "User-Agent as iPhone，but TCP trait more close to Linux (may is Android or 爬虫 library)"
.to_string(),
 30,
);
 }
 ...
 if ua.contains(" window s") && tcp_os.contains("linux") {
 // mayuseproxy or fingerprint混淆 not completely
 }
 }
 ...
 {
 report.add_discrepancy(
 "modern Chrome (120+) mustuse TLS 1.3，detect to protocol reduce level".to_string(),
 50,
);
 }
 ...
 // 3. Validateprotocol reduce levelabnormal
 if flow.context.protocol == fingerprint_core::system ::ProtocolType::Http
 && (flow.context.target_port == Some(443))
 {
 report.add_discrepancy(
 " in 443 portdetect to 明 text HTTP traffic (may is 强制protocol reduce levelattack)".to_string(),
 50,
);
 }

 // 4. JA4+ seriescrossValidate (more 深layerfingerprintconsistency)
 if let (Some(tls), Some(http)) = (tls_fingerprints.first(), http_fingerprints.first()) {
 ...
 report.add_discrepancy(
 format!("detect to modern TLS trait (JA4: {})，but HTTP behavior perform as traditional no Cookie request (JA4H: {})", ja4, ja4h),
 20,
);
 }
 ...
 // TLS negotiate h2，butactualsend HTTP/1.1
 report.add_discrepancy(
 "TLS handshakenegotiate h2，butactualrequestuse HTTP/1.1".to_string(),
 30,
);
 }
 }
