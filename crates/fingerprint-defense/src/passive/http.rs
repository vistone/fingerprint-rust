//! HTTP passivefingerprint identify
//!
//! implement HTTP request/responsepassivefingerprint identify。
...
pub struct HttpFingerprint {
 ...
 pub h2_ window _update: Option<u32>,
...
impl HttpAnalyzer {
 ...
 // matchsignature
 let (signature, similar ity) = self.match_signature(&request);
...
.collect();
 ...
 h2_ window _update: request.h2_ window _update,
 metadata,
 });
 }
 ...
 // limit parsed countdata量，preventoversized package causeinsidememory exhausted
 ...
 if stream_id == 0 {
 h2_ window _update = Some(increment);
 }
 }
 }
 _ => {}
 ...
 if h2_settings.is_empty() && h2_ window _update.is_none() {
 return None;
 }
