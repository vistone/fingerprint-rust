...
impl AdvancedStatistics {
    ...
    pub fn benfords_law_test(values: &[u32]) -> f64 {
        ...
        let observed: Vec<f64> = counts.iter()
            .map(|&c| c as f64 / total as f64)
            .collect();
        
        // Chi-square test
        let chi_square: f64 = expected.iter()
            .zip(observed.iter())
            .map(|(&exp, &obs)| {
                let diff = obs - exp;
                (diff * diff) / exp
    ...
    pub fn chi_square_uniformity_test(distribution: &[u32], expected_uniform: bool) -> f64 {
        ...
        let chi_square: f64 = distribution.iter()
            .map(|&observed| {
    ...
    pub fn explain_detection(
        ...
        for (algorithm, &score) in algorithm_results {
            let description = if score > threshold {
                format!("{} indicates strong AI patterns (score: {:.2})", algorithm, score)
            } else {
        ...
        let primary_reason = if let Some((algo, score, _)) = factors.first() {
            if *score > threshold {
                format!("Primary indicator: {} (confidence: {:.1}%)", algo, score * 100.0)
            } else {
        ...
        let high_scores = factors.iter().filter(|(_, score, _)| *score > threshold).count();
        let low_scores = factors.iter().filter(|(_, score, _)| *score <= threshold).count();
...
mod tests {
    ...
    fn test_benfords_law_uniform() {
        ...
        assert!(score > 0.3, "Uniform data should have elevated AI score, got {}", score);
    }
    ...
    fn test_chi_square_uniformity() {
        ...
        assert!(score > 0.8, "Uniform distribution should score high for uniformity");
        ...
        assert!(score < 0.5, "Varied distribution should score low for uniformity");
    }
    ...
    fn test_calibrate_confidence() {
        ...
        assert!(image_conf > text_conf, "Image should have higher confidence range");
