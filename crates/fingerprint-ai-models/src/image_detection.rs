...
pub fn detect_ai_image_from_metadata(
    ...
    // Model attribution
    let model_probs = attribute_to_image_models(
        noise_consistency,
        texture_regularity,
        &patterns,
        &metadata,
    ...
    if let Some(size) = metadata.file_size {
        ...
        if ratio < 0.5 {
            patterns.push(ImagePattern {
                ...
                description: "Unusually efficient compression suggests synthetic origin".to_string(),
                confidence: 0.5,
                location: None,
            });
        }
...
fn calculate_noise_consistency(metadata: &ImageMetadata, patterns: &[ImagePattern]) -> f32 {
    ...
    if patterns.iter().any(|p| p.pattern_type == ImagePatternType::DiffusionNoise) {
...
fn detect_edge_artifacts(_metadata: &ImageMetadata, patterns: &[ImagePattern]) -> f32 {
    ...
    if patterns.iter().any(|p| p.pattern_type == ImagePatternType::GANCheckerboard) {
        artifact_score = 0.7;
    }
    
    // Boundary inconsistencies
    if patterns.iter().any(|p| p.pattern_type == ImagePatternType::BoundaryInconsistencies) {
        artifact_score += 0.2;
    }
...
fn attribute_to_image_models(
    ...
    let dalle3_score = if width == 1024 && height == 1024 && !patterns.iter().any(|p| p.pattern_type == ImagePatternType::TextArtifacts) {
    ...
    probabilities.insert("midjourney_v6".to_string(), midjourney_score.clamp(0.0, 1.0));
...
mod tests {
    ...
    fn test_pattern_detection() {
        ...
        assert!(patterns.iter().any(|p| p.pattern_type == ImagePatternType::SpectralAnomalies));
    }
