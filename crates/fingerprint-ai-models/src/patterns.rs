...
use crate::{providers::ProviderCharacteristics, AiProvider, AiProviderFingerprint};
use std::collections::HashMap;

/// Detect AI provider from API endpoint pattern
pub fn detect_from_endpoint(
    endpoint: &str,
    ...
    for provider_chars in providers {
        if provider_chars.matches_endpoint(endpoint) {
            ...
            let has_auth_header = provider_chars
                .auth_headers
                .iter()
                .any(|h| headers.contains_key(*h));

            let has_custom_header = provider_chars
                .custom_headers
                .iter()
                .any(|h| headers.contains_key(*h));
...
pub fn analyze_request_pattern(endpoints: &[(&str, u64)], // (endpoint, timestamp)
) -> f32 {
