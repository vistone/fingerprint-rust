...
fn detect_boundary_artifacts(metadata: &VideoMetadata, patterns: &[VideoPattern]) -> f32 {
...
fn attribute_to_video_models(
    ...
    let avatar_score = if has_face_content && patterns.iter().any(|p| p.pattern_type == VideoPatternType::FaceBoundaryBlur) {
...
mod tests {
    ...
    fn test_pattern_detection() {
        ...
        assert!(patterns.iter().any(|p| p.pattern_type == VideoPatternType::TemporalArtifacts));
    }
