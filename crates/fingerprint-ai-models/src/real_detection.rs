...
impl RealImageAnalyzer {
    ...
    fn analyze_noise_patterns(img: &RgbImage) -> f64 {
        ...
        let variance_of_variance: f64 = variances
            .iter()
            .map(|v| (v - mean).powi(2))
            .sum::<f64>()
    ...
    fn analyze_metadata(file_data: &[u8], format: &str) -> f64 {
        ...
        if format == "PNG" {
            if file_data.windows(4).any(|w| w == b"tEXt" || w == b"iTXt") {
                ...
                if data_str.contains("AI") || data_str.contains("Generated")
                    || data_str.contains("DALL") || data_str.contains("Stable Diffusion")
...
mod tests {
    ...
    fn test_noise_pattern_analysis() {
        ...
        assert!(uniformity >= 0.0 && uniformity <= 1.0, "Uniformity should be in range [0,1]: {}", uniformity);
    }
    ...
    fn test_analyze_high_quality_photo() {
        ...
        assert!(noise >= 0.0 && noise <= 1.0, "Varied photo uniformity should be in range: {}", noise);
    }
    ...
    fn test_analyze_ai_generated_image() {
        ...
        assert!(noise >= 0.0 && noise <= 1.0, "Uniform pattern uniformity should be in range: {}", noise);
    }
}
