...
use fingerprint_ai_models::model_fingerprints::{
    FingerprintLearner, ModelFingerprintDatabase,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Total samples: {}", fingerprints.iter().map(|fp| fp.sample_count).sum::<usize>());
    ...
    for fp in &fingerprints {
        let icon = if fp.model_type == "text" { "📝" } else { "🎨" };
        ...
        if fp.model_type == "text" {
            println!("  │  ├─ Perplexity: {:.3} ± {:.3}", 
                fp.statistical_signature.perplexity_mean,
                fp.statistical_signature.perplexity_std);
            println!("  │  ├─ Burstiness: {:.3} ± {:.3}", 
                fp.statistical_signature.burstiness_mean,
                fp.statistical_signature.burstiness_std);
            println!("  │  └─ Vocabulary: {:.3} ± {:.3}", 
                fp.statistical_signature.vocabulary_richness_mean,
                fp.statistical_signature.vocabulary_richness_std);
        } else {
            println!("  │  ├─ Noise patterns: {:.3} ± {:.3}", 
                fp.statistical_signature.noise_pattern_mean,
                fp.statistical_signature.noise_pattern_std);
            println!("  │  ├─ Texture regularity: {:.3} ± {:.3}", 
                fp.statistical_signature.texture_regularity_mean,
                fp.statistical_signature.texture_regularity_std);
            println!("  │  └─ Color distribution: {:.3} ± {:.3}", 
                fp.statistical_signature.color_distribution_mean,
                fp.statistical_signature.color_distribution_std);
        }
        
        println!("  └─ Characteristic Patterns: {}", fp.characteristic_patterns.len());
        println!();
    }
    ...
    println!("     - text: {} models", db.get_models_by_type("text").len());
    println!("     - image: {} models", db.get_models_by_type("image").len());
    println!();
    ...
    if let Some(test_fp) = loaded_db.get_fingerprint("gpt4") {
        ...
        if let Some((best_model, similarity)) = matches.first() {
            println!("  ✓ Best match found: {} (similarity: {:.1}%)", best_model, similarity * 100.0);
        }
    }
