...
- **JA4+ 全栈指纹**: 集成 JA4 (TLS), JA4H (HTTP), JA4T (TCP) 的产生与识别
- **跨层一致性审计**: 检测 User-Agent 与底层 TCP 协议栈、TLS 版本之间的不一致性
- **指纹自学习**: 自动识别并记录未知的稳定指纹特征，对抗 0-day 机器人
- **持久化分析**: 基于 SQLite 的流量与威胁分析数据库
- **实时捕获**: 支持通过 Pcap 进行网卡实时流量审计
