...
NAMESPACE_CACHING="${NAMESPACE_CACHING:-caching}"
NAMESPACE_API="${NAMESPACE_API:-fingerprint-api}"
NAMESPACE_MONITORING="${NAMESPACE_MONITORING:-monitoring}"
REDIS_SERVICE_HOST="${REDIS_SERVICE_HOST:-redis-cluster.caching}"
REDIS_SERVICE_PORT="${REDIS_SERVICE_PORT:-6379}"
REDIS_SENTINEL_PORT="${REDIS_SENTINEL_PORT:-26379}"
REDIS_PORT_FORWARD_PORT="${REDIS_PORT_FORWARD_PORT:-6379}"
CACHE_REDIS_ADDR="${CACHE_REDIS_ADDR:-${REDIS_SERVICE_HOST}:${REDIS_SERVICE_PORT}}"
...
deploy_redis_cluster() {
  ...
  REPLICAS=$(
    kubectl exec -n "$NAMESPACE_CACHING" "$MASTER_POD" -- redis-cli info replication |
...
update_api_deployment() {
  ...
  if kubectl get deployment fingerprint-api -n "$NAMESPACE_API" > /dev/null 2>&1; then
...
verify_redis_cluster() {
  ...
  kubectl exec -n "$NAMESPACE_CACHING" "$REDIS_POD" -c sentinel -- \
    redis-cli -p "$REDIS_SENTINEL_PORT" sentinel masters | grep -q mymaster && \
  ...
  SCRAPE_OK=$(kubectl logs -n "$NAMESPACE_MONITORING" -l app.kubernetes.io/name=prometheus --tail=50 2>/dev/null | grep -c "redis" || true)
...
verify_monitoring() {
  ...
  kubectl get servicemonitor redis -n "$NAMESPACE_CACHING" > /dev/null 2>&1 && \
    log_info "✓ Redis ServiceMonitor created" || \
    log_warn "⚠ Redis ServiceMonitor not found"
  
  # Check PrometheusRule
  kubectl get prometheusrule redis-caching -n "$NAMESPACE_MONITORING" > /dev/null 2>&1 && \
    log_info "✓ Redis PrometheusRule created" || \
    log_warn "⚠ Redis PrometheusRule not found"
  
  # Check Grafana dashboards
  kubectl get cm cache-performance-dashboard -n "$NAMESPACE_MONITORING" > /dev/null 2>&1 && \
...
verify_cache_management() {
  ...
  # Check CronJobs
  CRONJOB_COUNT=$(kubectl get cronjob -n "$NAMESPACE_API" --no-headers 2>/dev/null | wc -l)
  ...
  kubectl get deployment cache-invalidation-watcher -n "$NAMESPACE_API" > /dev/null 2>&1 && \
...
test_cache() {
  ...
  kubectl port-forward -n "$NAMESPACE_CACHING" svc/redis-cluster "${REDIS_PORT_FORWARD_PORT}:${REDIS_SERVICE_PORT}" > /dev/null 2>&1 &
  pf_pid=$!
  sleep 2
  
  # Test set/get
  if redis-cli -h 127.0.0.1 -p "$REDIS_PORT_FORWARD_PORT" SET test:key "test:value" > /dev/null 2>&1 && \
     redis-cli -h 127.0.0.1 -p "$REDIS_PORT_FORWARD_PORT" GET test:key 2>/dev/null | grep -q "test:value"; then
    log_info "✓ Redis basic operations working"
    redis-cli -h 127.0.0.1 -p "$REDIS_PORT_FORWARD_PORT" DEL test:key > /dev/null 2>&1
  else
...
main() {
  ...
  log_info ""
  log_info "Next steps:"
  log_info "1. Monitor Redis cluster: kubectl port-forward -n $NAMESPACE_CACHING svc/redis-cluster ${REDIS_PORT_FORWARD_PORT}:${REDIS_SERVICE_PORT}"
