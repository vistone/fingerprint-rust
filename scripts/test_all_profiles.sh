...
for profile in "${PROFILES[@]}"; do
    printf "测试 %-30s " "${profile}..."
    
    # 运行导出命令，捕获输出
    OUTPUT=$(cargo run --example export_config --features export "$profile" "exported_profiles/${profile}.json" 2>&1)
    EXIT_CODE=$?
...
# 显示导出的文件统计
echo "导出的文件:"
ls -lh exported_profiles/*.json 2>/dev/null | wc -l | xargs echo "  文件数量:"
du -sh exported_profiles 2>/dev/null | awk '{print "  总大小: " $1}'
