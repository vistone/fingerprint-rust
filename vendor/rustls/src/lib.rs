...
pub mod client {
    ...
    impl ClientHello<'_> {
        ...
        /// Returns the current encoded length (in bytes) of the ClientHello payload.
        ///
        /// This is useful when computing padding length strategies.
        ...
        /// Remove a ClientHello extension by type.
        ///
        /// Returns `true` if an extension was removed.
        pub fn remove_extension(&mut self, typ: ExtensionType) -> bool {
            if let Some(pos) = self.inner.extensions.iter().position(|e| e.get_type() == typ) {
                self.inner.extensions.remove(pos);
                true
            } else {
                false
            }
        ...
        /// Append a raw (possibly unknown) extension to ClientHello.
        ///
        /// The extension `typ` must not already be present, otherwise an error is returned.
        pub fn push_raw_extension(
            &mut self,
            typ: ExtensionType,
            payload: Vec<u8>,
        ) -> Result<(), crate::Error> {
            ...
            if self.inner.extensions.iter().any(|e| e.get_type() == typ) {
                return Err(crate::Error::General(
                    "duplicate ClientHello extension type".into(),
                ));
            }

            self.inner.extensions.push(ClientExtension::Unknown(UnknownExtension {
                typ,
        ...
        /// Set the payload of the `supported_groups` (elliptic_curves) extension.
        ///
        /// `groups` are IANA NamedGroup IDs (u16). Unknown values are allowed.
        pub fn set_named_groups(&mut self, groups: Vec<u16>) -> Result<(), crate::Error> {
            ...
            let Some(ext) = self
                ...
                .extensions
                .iter_mut()
                .find(|e| e.get_type() == ExtensionType::EllipticCurves)
            else {
                return Err(crate::Error::General(
                    "ClientHello missing supported_groups extension".into(),
                ));
            };

            match ext {
                ClientExtension::NamedGroups(ref mut v) => {
                    *v = groups.into_iter().map(NamedGroup::from).collect();
                    Ok(())
                }
                _ => Err(crate::Error::General(
                    "unexpected supported_groups extension variant".into(),
                )),
            }
        }

        /// Read the payload of the `supported_groups` (elliptic_curves) extension as u16 IDs.
        pub fn named_groups(&self) -> Option<Vec<u16>> {
            use crate::msgs::handshake::ClientExtension;
            let ext = self
                .inner
                .extensions
                ...
                .find(|e| e.get_type() == ExtensionType::EllipticCurves)?;
            match ext {
                ClientExtension::NamedGroups(v) => Some(v.iter().map(|g| g.get_u16()).collect()),
                _ => None,
            }
        }

        /// Set the payload of the `key_share` extension.
        ///
        /// `entries` are (NamedGroup ID, key_exchange_bytes).
        pub fn set_key_share_entries(
            &mut self,
            entries: Vec<(u16, Vec<u8>)>,
        ) -> Result<(), crate::Error> {
            use crate::msgs::enums::NamedGroup;
            use crate::msgs::handshake::{ClientExtension, KeyShareEntry};

            let Some(ext) = self
                .inner
                .extensions
                .iter_mut()
                .find(|e| e.get_type() == ExtensionType::KeyShare)
            else {
                return Err(crate::Error::General(
                    "ClientHello missing key_share extension".into(),
                ));
            };

            match ext {
                ClientExtension::KeyShare(ref mut v) => {
                    *v = entries
                        .into_iter()
                        .map(|(group, data)| KeyShareEntry::new(NamedGroup::from(group), &data))
                        .collect();
                    Ok(())
                }
                _ => Err(crate::Error::General(
                    "unexpected key_share extension variant".into(),
                )),
            }
        }

        /// Read the payload of the `key_share` extension as (group_id, key_exchange_bytes).
        pub fn key_share_entries(&self) -> Option<Vec<(u16, Vec<u8>)>> {
            use crate::msgs::handshake::ClientExtension;
            let ext = self
                .inner
                .extensions
                ...
                .find(|e| e.get_type() == ExtensionType::KeyShare)?;
            match ext {
                ClientExtension::KeyShare(v) => Some(
                    v.iter()
                        .map(|e| (e.group.get_u16(), e.payload.0.clone()))
                        .collect(),
                ),
                _ => None,
            }
